  ...
  <StudentLayout>
    ...
    <div class="max-w-5xl mx-auto py-10 px-6 space-y-6">
      ...
      >
        ...
        <!-- Kursa Katıl / Kurstan Çık butonları -->
...
const enrolled = ref(props.enrolled || false)
...
function enroll() {
  inertiaPost(route('student.courses.enroll', course.id), {}, {
    ...
    onError: () => studentStore.showToast('❌ Kursa kayıt başarısız.', 'error'),
  })
}
...
function performUnenroll() {
  inertiaDelete(route('student.courses.unenroll', course.id), {
    onSuccess: () => {
      ...
      // Çıkıştan sonra Kurslarım sayfasına yönlendir
